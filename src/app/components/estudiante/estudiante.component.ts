...
import { FormEstudiantesComponent } from 'src/app/Form/form-estudiantes/form-estudiantes.component';
...
export class EstudianteComponent implements OnInit, AfterViewInit {
  ...
  openDialog() {
    const dialogRef = this.dialog.open(FormEstudiantesComponent);
  }
  ...
  public crearEstudiantes(nuevoEstudiante : any) {
  ...
  public actualizarEstudiantes(idestudiates: number) { 
    const newData = { /* tus datos a actualizar */ };

    // Llama al método Put del servicio RestService.
    this.api.Put("Estudiantes", idestudiates, newData);
  }
